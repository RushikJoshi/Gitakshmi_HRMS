...
const fs = require('fs');
const logFile = './test_offer_persistence.txt';
const log = (msg) => {
    console.log(msg);
...
const runTest = async () => {
    ...
    try {
        ...
        const testApplicant = await Applicant.create({
            name: 'Test Offer Persistence',
            email: `test_offer_${Date.now()}@example.com`,
            mobile: '1234567890',
            experience: '5',
        ...
        // Test offer letter generation with joiningDate and address
        log(`\n--- TEST: Offer Letter Generation with Data Persistence ---`);

        const testJoiningDate = '2024-02-15';
        const testAddress = '456 Test Avenue, Test City, Test State 12345';
        ...
        require('./services/letterPDFGenerator').generatePDF = async () => {
            return { filename: 'test_offer.pdf' };
        };
        ...
        if (res.statusCode === 200 && res.data.success) {
            ...
            // Check if joiningDate and address were persisted
            ...
            if (updatedApplicant.offerRefCode === 'OFFER/2024/TEST123') {
                log('✅ PASS: Offer ref code persisted correctly');
            } else {
                log(`❌ FAIL: Offer ref code not persisted. Expected: "OFFER/2024/TEST123", Got: "${updatedApplicant.offerRefCode}"`);
            }
