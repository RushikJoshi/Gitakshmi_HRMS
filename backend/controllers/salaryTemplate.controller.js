...
exports.previewTemplate = async (req, res) => {
    try {
        ...
        if (overrideAnnualCTC && !isNaN(overrideAnnualCTC) && overrideAnnualCTC > 0) {
            try {
                calculated = await salaryCalculationService.calculateSalaryStructure(
                    overrideAnnualCTC,
                    template.earnings || [],
                    ...
                    template.employerDeductions || [],
                    req.tenantDB,
                    tenantId
                );
            } catch (calcErr) {
                console.error('[PREVIEW_TEMPLATE] Override calculation failed:', calcErr);
                // Fallback to default template calculation
                calculated = await salaryCalculationService.calculateCompleteSalaryBreakdown(template);
            }
        } else {
            // Use default template calculation
            try {
                calculated = await salaryCalculationService.calculateCompleteSalaryBreakdown(template);
            } catch (calcErr) {
                console.error('[PREVIEW_TEMPLATE] Calculation failed:', calcErr);
                return res.status(400).json({ success: false, error: `Calculation failed: ${calcErr.message}` });
            }
        }

        if (!calculated) {
            return res.status(400).json({ success: false, error: 'Failed to calculate salary breakdown' });
        }
        ...
        const salarySnapshot = {
            ...
            employerContributions: calculated.employerContributions || calculated.benefits || [],
            employeeDeductions: calculated.employeeDeductions || calculated.deductions || [],
            grossA: calculated.grossA || { monthly: 0, yearly: 0 },
            grossB: calculated.grossB || { monthly: 0, yearly: 0 },
            grossC: calculated.grossC || { monthly: 0, yearly: 0 },
            takeHome: calculated.takeHome || { monthly: 0, yearly: 0 },
            ctc: calculated.ctc || { monthly: 0, yearly: 0 },
