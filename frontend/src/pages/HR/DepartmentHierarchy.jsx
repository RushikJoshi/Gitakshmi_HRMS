...
export default function DepartmentHierarchy() {
  ...
  async function loadHierarchy() {
    try {
      ...
      // Organize by Department → Manager → Employees
      const organized = {};
      ...
      <div className="bg-white rounded-lg border-2 border-slate-300 shadow-lg overflow-hidden">
        ...
        <div className="divide-y">
          {departments.map((dept) => {
            ...
            const isDeptExpanded = expandedDepts[dept] !== false; // Default to expanded
            ...
            return (
              <div key={dept} className="border-l-4 border-l-purple-300">
                ...
                >
                  ...
                  <div className="w-14 h-14 rounded-lg bg-purple-100 text-purple-600 flex items-center justify-center text-xl font-bold flex-shrink-0">
                    📁
                  </div>
                          ...
                          >
                            ...
                            <div className="flex-grow text-left">
                              <h4 className="font-semibold text-slate-900">
                                {manager.managerName || 'Unassigned Manager'}
                              </h4>
                              <p className="text-sm text-slate-600">
                                {manager.managerRole} • {empCount} {empCount === 1 ? 'employee' : 'employees'}
                              </p>
                            </div>
                          </button>
                              ...
                              {empCount === 0 ? (
                                ...
                                <div className="divide-y">
                                  {manager.employees.map((emp) => (
                                    <div
                                      key={emp._id}
                                      className="p-5 hover:bg-white transition flex items-center gap-4 border-l-4 border-l-slate-300"
                                    >
                                      <div className="w-10 h-10 rounded-full bg-slate-300 text-slate-700 flex items-center justify-center font-bold flex-shrink-0">
                                        {`${(emp.firstName || '').slice(0, 1)}${(emp.lastName || '').slice(0, 1)}`.toUpperCase() || '?'}
                                      </div>
                                      <div className="flex-grow">
                                        ...
                                        <p className="text-sm text-slate-600">
                                          {emp.employeeId} • {emp.role || 'Employee'}
                                        </p>
                                      </div>
                                      <div className="text-xs bg-white px-3 py-1 rounded-full text-slate-600">
                                        {emp.email}
                                      </div>
                                    </div>
                                  ))}
        ...
        <div className="bg-slate-50 border-t p-6">
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-slate-900">
                {Object.values(data).reduce((sum, dept) => sum + Object.keys(dept).length, 0)}
              </div>
              <div className="text-sm text-slate-600">Managers</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-slate-900">
                {Object.values(data).reduce((sum, dept) => sum + Object.values(dept).reduce((s, m) => s + m.employees.length, 0), 0)}
              </div>
              <div className="text-sm text-slate-600">Employees</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-slate-900">
                {Object.values(data).reduce((sum, dept) => sum + Object.values(dept).reduce((s, m) => s + m.employees.length, 0), 0) + Object.values(data).reduce((sum, dept) => sum + Object.keys(dept).length, 0)}
              </div>
