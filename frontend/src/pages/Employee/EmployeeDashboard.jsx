...
export default function EmployeeDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [profileRes, attRes, leaveRes, balanceRes, holidayRes, settingsRes] = await Promise.all([
      ...
      const leavesTaken = balanceRes.data?.reduce((sum, b) => sum + (b.used || 0), 0) || 0;
  ...
  const handleClockInOut = async () => {
    try {
      ...
      alert(err.response?.data?.error || "Failed context policy check");
    } finally {
      setClocking(false);
    }
  };
  ...
  const StatCard = ({ title, value, color, icon: Icon }) => (
    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none hover:translate-y-[-4px] transition-all duration-300">
      <div className="flex justify-between items-start mb-4">
        <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{title}</div>
        {Icon && <Icon className={`${color} opacity-20`} size={24} />}
      </div>
      <div className={`text-5xl font-black ${color} tracking-tighter`}>{value}</div>
      <div className={`h-1.5 w-12 rounded-full mt-4 bg-current ${color} opacity-20`}></div>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            />
            <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-8 rounded-3xl shadow-xl shadow-blue-500/20 text-white flex flex-col justify-between overflow-hidden relative">
              <div className="absolute top-[-10px] right-[-10px] opacity-10 transform rotate-12">
                <CalendarIcon size={120} />
              </div>
              <div className="relative z-10">
                <div className="text-[10px] font-black uppercase tracking-widest opacity-70 mb-4">Upcoming Holiday</div>
                {stats.nextHoliday ? (
                  <>
                    <div className="text-xl font-black leading-tight truncate">{stats.nextHoliday.name}</div>
                    <div className="text-sm font-bold opacity-80 mt-1">
                      {formatDateDDMMYYYY(stats.nextHoliday.date)}
                    </div>
                  </>
                ) : (
                  <div className="text-sm font-bold">No upcoming holidays</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl overflow-hidden relative border border-slate-800">
                ...
                <div className="relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Shift Timings</p>
                      <p className="text-sm font-black text-indigo-400">{attendanceSettings?.shiftStartTime} - {attendanceSettings?.shiftEndTime}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Punch Mode</p>
                      <p className="text-sm font-black text-blue-400 uppercase tracking-tight">{attendanceSettings?.punchMode} Punch</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Late Threshold</p>
                      <p className="text-sm font-black text-emerald-400">{attendanceSettings?.lateMarkThresholdMinutes} Minutes</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Working Target</p>
                      <p className="text-sm font-black text-amber-400">{attendanceSettings?.fullDayThresholdHours} Hours / Day</p>
                    </div>
                  </div>
                  <div className="mt-6 pt-4 border-t border-slate-800/50 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Location Restriction</span>
                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${attendanceSettings?.locationRestrictionMode === 'none' ? 'bg-slate-800 text-slate-400' : 'bg-emerald-500/20 text-emerald-500'}`}>
              ...
              <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <AlertCircle size={14} className="text-blue-500" />
                    Recent Activity
                  </h3>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-slate-900 px-2 py-1 rounded-lg">Last 5 Days</span>
                </div>

                <div className="space-y-4">
                  {attendance.slice(0, 5).map(att => (
                    <div key={att._id} className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-all border border-transparent hover:border-slate-100 dark:hover:border-slate-800">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <ReportingTree />
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none flex flex-col justify-center items-center text-center">
              <Users className="text-blue-500 opacity-20 mb-4" size={48} />
              <h4 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-widest">Connect with your Team</h4>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'leaves' && (
        <div className="space-y-6">
              ...
              {balances.map(b => {
                ...
                return (
                  <div key={b._id} className="relative overflow-hidden group">
                    ...
                    <div className="relative bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none">
                      <div className="flex justify-between items-start mb-4">
                        <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{b.leaveType}</div>
                        <div className={`h-2 w-2 rounded-full bg-gradient-to-r ${gradient}`}></div>
                      </div>
                      <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-black text-slate-800 dark:text-white tracking-tighter">{b.available || 0}</span>
                        <span className="text-sm font-bold text-slate-400">/ {b.total || 0}</span>
                      </div>
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Available Days</div>

                      <div className="mt-6 pt-4 border-t border-slate-50 dark:border-slate-700/50 grid grid-cols-2 gap-2">
                        <div>
          ...
          {balances.length === 0 && (
            <div className="p-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900 rounded-xl text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-6">Leave History</h3>
